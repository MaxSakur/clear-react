...
import { Burger, Logo } from "../../components/UI";
import VerticalCarousel from "../../components/VerticalCarousel";
import Card from "../../components/Card";
import Menu from "../../components/Menu";
import {
  PTACTICE_TEXTAREA_PLACEHOLDER,
...
export const MainPage = ({ width, itemsArray }) => {
  const activeCategory = useSelector(
    (store) => store.categoryList.activeCategoryIndex
  );
  ...
  const testFunc = (arr, activeI, text) => {
    const test2 = arr.map((el) =>
      el.id === [activeI] ? (el.practice = text) : el
    );
    console.log(test2);
  };
      ...
      <div className={style.mainPage_wrapper}>
        {wordsArray[activeCategory] ? (
          wordsArray[activeCategory].map((word, index) => (
            <Card key={index + word} title={word} />
          ))
        ) : (
          <>Массив пуст</>
        )}
